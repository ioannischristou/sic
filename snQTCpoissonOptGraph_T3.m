function [sopt Qopt Topt costs] = snQTCpoissonOptGraph_T3(Tmax,Kr,K0,L,lamda,h,p, epsq, epst, tnot, p2, estop)
if nargin < 12
    estop=0;
end
if nargin < 11
...
lb_Song_Ti = 1:Tlen;  % Lower Bound proposed by Song in good-news-bad-news: 
% works only when there are no fixed-costs
copt_tot = 10.0^30;
...
for i=1:Tlen
    ...
    Ti(i)=(i-1)*epst+tnot;
    T = Ti(i);
    lb_Song_Ti(i) = h*p*lamda*T/(2*(h+p));
    disp(['T=' num2str(T)]);
    j=1;
    while true
        j = j+1;
        ...
        if hsmall > copt && -Q<estop  % if estop is negative, run Q-loop -stop times
            break;
        end
        if estop==1
...
%hold off
hold on
plot(Ti(1:Tlen),lb_Song_Ti(1:Tlen),'g-');
hold off
%hold on
