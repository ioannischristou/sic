function [sopt qopt topt copt] = snQTCnbinOptFastApprox5(Kr, K0, L, lamda, pl, h, p, p2, tmin, tmax, epsq, epst)
...
% itc20180910: options used to be as commented out
...
s0 = round(-lamda*pl/((1-pl)*log(1-pl)));
x0 = [s0 epsq];
tryorder = [1 2];
epsx = [1 epsq];
[vT z] = nonlinoptAD(@(x) f22P(x, T, Kr, K0, L, lamda, pl, h, p, epsq), x0, tryorder, epsx);
end


function z = f22P(x, T, Kr, K0, L, lamda, pl, h, p, epsq)
mean = -lamda*pl/((1-pl)*log(1-pl));
sT = x(1);
QT = x(2);
penalty = 0;
if (QT<epsq)
    penalty = (epsq-QT)*10000;
    QT = epsq;  % epsq is also the lower bound on Q
end
z = Kr/T+K0*mean/QT + snQTCnbin(sT,QT,T,0,0,L,lamda,pl,h,p);
% introduce a penalty for invalid QT
z = z + penalty;
end
