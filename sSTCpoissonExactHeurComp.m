...
fid = fopen('sSTCpoissonComp.new.txt', 'wt');
...
% Kr K0 h p s* S* T* c* time* s1 S1 T1 c1 time1 r1 r1+Q1 T1 c2 time2 

fprintf(fid, '    Kr ,     Ko ,      h ,      p ,    s* ,    S* ,    T* ,    c* , time*, s1 , S1 , T1 , c1 , time1, r1 , r1+Q1 , T1 , c2 , time2\n');
    ...
    for k=1:h_len
        ...
        for l=1:p_len
            ...
            for i=1:Kr_len
                ...
                tic;
                [sopt Sopt Topt copt] = sSTCpoissonOptFastApprox3(Kr, K0, L, lamda, h, p);
                tElapsed=toc;
                if copt < ci(i)  % round-off error
                    fprintf(fid,'%6.2f , %6.2f , %6.2f , %6.2f , %6.2f , %6.2f , %6.2f , %6.2f , %6.2f ,', Kr, K0, h, p, sopt, Sopt, Topt, copt, timesi(i));
                else
                    fprintf(fid,'%6.2f , %6.2f , %6.2f , %6.2f , %6.2f , %6.2f , %6.2f , %6.2f , %6.2f ,', Kr, K0, h, p, si(i), Si(i), Ti(i), ci(i), timesi(i));
                end
...
%                fprintf(fid,'%6.2f , %6.2f , %6.2f , %6.2f , %6.2f', sopt, Sopt, Topt, copt, tElapsed);
                 tic;
                 [ropt Qopt Topt c] = snQTCpoissonOptFastApprox3(Kr,K0,L,lamda,h,p);
                 s2 = ropt;
                 S2 = ropt+Qopt;
                 T2 = Topt;
                 c2 = sSTCpoisson(s2,S2,T2,Kr,K0,L,lamda,h,p);
                 tElapsed=toc;
