% comparison between exact (s,S,T) and SA algorithm

fid = fopen('sSTCnormApproxSA_200.txt', 'wt');
    ...
    for k=1:h_len
        ...
        for l=1:p_len
            ...
            for i=1:Kr_len
                Kr=Krs(i);
                tic;
                [xb yb xs k] = nonlinoptSA(@(v) sSTCnorm_2(v,Kr,K0,L,m,s,h,p,p2), x0, 200);  % default kmax=1000
                timesi=toc;
                si = xb(1,1);
                Si = xb(2,1);
                Ti = xb(3,1);
